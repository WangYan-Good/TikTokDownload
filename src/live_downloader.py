...
class LiveDownloader(Downloader):
  ...
  def listen_inactived_live(self):
     for url in self.download_url_list:
        ...
        ##
        if self.get_douyin_live_download_stream(url) == None:
           continue
        ##
        ...
        if self.actived_download_live_url_list.count(url) >= 1:
           if self.live_share_url_download_status_list[thread_index] is False:
            ...
            self.live_share_url_download_status_list[thread_index] = True
            self.live_download_thread_list[thread_index].start()
        else:
           ##
           ## add live download thread
           ##
           self.download_live_stream(url)
        ##
        ## threading related url does not in actived live list
        ## active live sub-thread
        ##
